...
void affichage(SDL_Texture* textureFond, SDL_Texture* textureSpriteDebout, SDL_Texture* textureSpriteDebout2, SDL_Texture* avancer1, SDL_Texture* avancer2, SDL_Texture* avancer3, SDL_Texture* avancer4,
    ...
    SDL_Rect* barreDeVieRedDroite, SDL_Rect* rectanglePerso1, SDL_Rect* destRectDebout, SDL_Rect* destRectAccroupis, SDL_Rect* destRectCoupDebout, SDL_Rect* destRectAccroupis2, SDL_Rect* destRectBot,
    SDL_Rect* rectangleCoup, int* psnick, int* pcoup, int* pcoupPied, int* pcompteur, int* pavancer, int* pcompteurAvancer, int* pcompteursaut, int* psaut, SDL_Rect* rectanglePunchingBall, TTF_Font* font,
    const char* text, int x, int y,SDL_Color* colortimer)// est définie pour dessiner les éléments du jeu sur la fenêtre. Elle prend en paramètre la texture à afficher, le renderer, les rectangles de base et de coup, et le coup choisit.
{
    ...
    SDL_RenderCopy(renderer, deboutBot, NULL, destRectBot);// charge la texture qui sera mise aux coordonée de destRectBot1
    if (*psaut == 1 || *psaut == 2)
    {
        if (*pcompteursaut < 10) // permet de gerer l'animation compte le nombre de fois ou rentre dans la fonction pour gérer affichage 
        {
            SDL_RenderCopy(renderer, saut1, NULL, destRectDebout);// charge la texture qui sera mise aux coordonée de destRect2
            *pcompteursaut += 1;
        }
    ...
    else if (*pcoup != 0 && *psnick == 1)
    {
        ...
        {
            SDL_RenderCopy(renderer, coupAccroupis3, NULL, destRectAccroupis2);
        }
        else if (rectangleCoup->w > 30)
        {
    ...
    SDL_RenderDrawRect(renderer, rectanglePerso1);// dessine le rectangle pour la couleur choisit (SDL_RenderDrawRect rectangle vide,SDL_RenderFillRect rectangle plein)
...
void win(SDL_Rect* rectanglePerso1, SDL_Texture* texture, SDL_Texture* win1, SDL_Texture* win2, SDL_Texture* win3, SDL_Renderer* renderer, SDL_Rect* destRectDebout, SDL_Rect* destRectwin
    ...
    if (*petapeWin == 0) {
        ...
        *pcompteurWin += 1;
        ...
        SDL_RenderCopy(renderer, win3, NULL, destRectwin);
    }
    SDL_RenderPresent(renderer);// affiche le rendu
}
...
void jeu(int* pjouer) {
    ...
    SDL_Rect destRectAccroupis2;
    SDL_Rect destRectBot;
    SDL_Rect destRectwin;
    ...
    destRectBot.x = rectanglePunchingBall.x;// le rectangle ou on va afficher le sprite prend la valeur du rectangle de la hitbox punchingball
    destRectBot.y = rectanglePunchingBall.y;
    ...
    SDL_QueryTexture(coupAccroupis3, NULL, NULL, &destRectAccroupis2.w, &destRectAccroupis2.h);
    SDL_QueryTexture(deboutBot, NULL, NULL, &destRectBot.w, &destRectBot.h);
    SDL_QueryTexture(win3, NULL, NULL, &destRectwin.w, &destRectwin.h);
    ...
    while (jeu == 0)
    {
        ...
        destRectAccroupis2.x = rectanglePerso1.x;
        destRectAccroupis2.y = rectanglePerso1.y;
        destRectwin.x = rectanglePerso1.x;
        destRectwin.y = rectanglePerso1.y;
        ...
        if (barreDeVieDroite.w <= 0 || minuteur <= 0) { // regarde si a gagner
            win(&rectanglePerso1, textureFond, win1, win2, win3, renderer, &destRectDebout, &destRectwin, &etape, &compteurWin);// affichage si a gagner
        }
        else {
            affichage(textureFond, textureSpriteDebout, textureSpriteDebout2, avancer1, avancer2, avancer3, avancer4, avancer5, accroupis, saut1, saut2, saut3, saut4, saut5, saut6, coupDebout1, coupDebout2,
                coupDebout3, coupAccroupis1, coupAccroupis2, coupAccroupis3, coupPied1, coupPied2, coupPied3, deboutBot, renderer, &barreDeVie, &barreDeVieDroite, &barreDeVieRed, &barreDeVieRedDroite,&rectanglePerso1
                , &destRectDebout, &destRectAccroupis, &destRectCoupDebout, &destRectAccroupis2, &destRectBot, &rectangleCoup, &snick, &coup,&coupPied, &compteur, &avancer, &compteurAvancer, &compteurSaut, &saut,
                &rectanglePunchingBall, font, timer_text, 900, 70, &color_timer); // affichage si a gagner
        }
